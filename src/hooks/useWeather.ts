...
interface WeatherStore {
  ...
  loading: boolean;
  lastUpdate: number;
  /** Bumped by both refresh() and pageRefresh() to trigger re-render */
  refreshTick: number;

  /** Fetch weather data from API — triggers re-render on completion */
  refresh: (city: string, apiKey: string) => Promise<void>;
  /** Bump tick to trigger re-render (time update, no weather fetch) */
  pageRefresh: () => void;
}
