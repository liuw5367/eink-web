...
15min ─── 两个定时器同时触发 ─────────────────────────────
         ...
         注: 两次刷新间隔取决于网络请求延迟，e-ink 上可能看到双重闪屏
         建议: 错开 interval 和 weatherInterval 避免同时触发
...
- 两者不共享状态，不互相等待
- 当 `interval === weatherInterval` 时，两个定时器同时触发会产生两次刷屏，建议错开配置

**典型配置组合**：

| interval | weatherInterval | 效果 |
|----------|----------------|------|
| 15min | 15min | 每 15 分钟同时更新天气和页面（⚠️ 可能双重闪屏） |
| 15min | 30min | 每 15 分钟刷新页面，天气每 30 分钟更新（推荐，避免同时触发） |
